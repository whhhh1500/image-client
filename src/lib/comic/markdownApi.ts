...
/** Canonical comic record projected for the catalog; it deliberately is not a generic assets row. */
export interface ComicCatalogEntry { sourceUri: string; projectId: string; kind: "text" | "image"; novelWorkId: string; novelChapterId?: string; chapterNo?: number; chapterTitle?: string; documentId?: string; documentRevision?: number; documentKind?: string; pageNo?: number; title: string; text?: string; /** Persisted page Markdown revision; source material, not necessarily the rendered request. */ sourcePrompt?: string; /** Exact persisted provider request. Missing means a legacy row has no complete prompt snapshot. */ effectivePrompt?: string; promptSnapshotComplete: boolean; path?: string; createdAt: number; stale: boolean }
...
export const comicMdCatalogList = (input: { projectId: string }) => call<ComicCatalogEntry[]>("comic_md_catalog_list", input);
